...
"""
Highest-level
recpack.metrics.ndcg_score(y_true, y_pred)
...
Ge kunt resultaten chainen, genre pandas:

dcg.normalize().groupby("item").mean()

-> Normalize = bad idea. Gewoon in de metric steken

Resultaten = Pandas DataFrame. Attribuut van metric. ".results"
Mean resultaat zoals conventioneel berekend: ".value"

"Utilities" bovenop results om al metrics te aggregeren? -> Doen we voorlopig niet. 
Definieren als een hoop functies in een soort van "visualize" module. 

ElementwiseMetric: DCG, Recall
ListwiseMetric: Diversity, nDCG, RR
GlobalMetric: Coverage

MetricK? FittableMetric? 

Recall.normalize() -> Normalized Recall
Recall: (predictions /\ relevant docs) @K / (relevant docs)
Onze implementatie = normalized recall?: (predictions /\ relevant docs) @K / MIN(K, relevant docs)
