...
def test_create_temporal_csr_matrix_from_pandas_df():
    ...
    mat = recpack.helpers.create_temporal_csr_matrix_from_pandas_df(df, 'item_id', 'user_id', 'timestamp')
...
def test_create_csr_matrix_from_pandas_df():
    ...
    mat = recpack.helpers.create_csr_matrix_from_pandas_df(df, 'item_id', 'user_id')
...
def test_create_data_M_from_pandas_df():
    ...
    d = recpack.helpers.create_data_M_from_pandas_df(df, 'item_id', 'user_id', 'timestamp')
    assert d.timestamps is not None
    assert d.values is not None

    assert d.shape == (3, 4)

    d2 = recpack.helpers.create_data_M_from_pandas_df(df, 'item_id', 'user_id')
