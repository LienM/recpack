...
def test_greedy_intra_list_diversifier_no_div(pageviews):
    sim_algo = ItemKNN(K=2)
    sim_algo.fit(pageviews)

    div_ranker = GreedyIntraListDiversifier(sim_algo, K=3, div_rate=0)
    hist = scipy.sparse.csr_matrix(([1,1], ([0,0], [1, 2])), shape=(1,pageviews.shape[1]))
    scores = sim_algo.predict(hist)
    result = div_ranker.rank(scores, hist)


    np.testing.assert_array_equal(result.toarray(), scores.toarray())
