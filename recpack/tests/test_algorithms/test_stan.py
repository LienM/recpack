...
def mini_dataset():
    ...
    return InteractionMatrix(
        df, user_ix="uid", item_ix="iid", timestamp_ix="tst"
    )
...
def test_init(algo):
    assert (
        algo.identifier == "STAN(K=2,distance_from_match_decay=1.0,"
        "interaction_decay=1.0,session_decay=0.2)"
    )
...
def test_fit(algo, mini_training_dataset):
    ...
    expected_interaction_positions = np.array(
        [[1, 2, 3, 0], [0, 0, 1, 2], [0, 0, 0, 0], [0, 0, 0, 0]]
    )
    ...
    np.testing.assert_array_equal(
        algo.historical_session_timestamps_.A.T, expected_session_timestamps
    )


def test_compute_session_similarity(
    algo, mini_training_dataset, mini_test_dataset
):
    algo.fit(mini_training_dataset)

    sim = algo._compute_session_similarity(
        mini_test_dataset.last_timestamps_matrix
    )
    expected_similarity = [
        [0, 0, 0, 0],
        [0, 0, 0, 0],
...
def test_compute_session_weights(
    algo, mini_training_dataset, mini_test_dataset
):
    algo.fit(mini_training_dataset)
    ...
    session_weights = algo._compute_session_similarity_weights(
        mini_test_dataset.last_timestamps_matrix, sim
    )
    ...
    np.testing.assert_array_almost_equal(
        session_weights.toarray(), expected_weights
    )


def test_compute_prediction_scores(
    algo, mini_training_dataset, mini_test_dataset
):
    algo.fit(mini_training_dataset)
    ...
    # fmt: on
    predictions = algo._compute_prediction_scores(
        session_similarities, mini_test_dataset
    )
...
def test_compute_prediction_scores_single_neighbour(
    algo, mini_training_dataset, mini_test_dataset
):
    algo.fit(mini_training_dataset)
...
def test_predict(algo, mini_training_dataset, mini_test_dataset):
    ...
    np.testing.assert_array_almost_equal(
        predictions.toarray(), expected_predictions
    )
