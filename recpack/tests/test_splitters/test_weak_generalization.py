...
def test_weak_generalization_invalid_perc(data_m):
    with pytest.raises(AssertionError):
        scenarios.WeakGeneralization(
            0.7, perc_interactions_validation=0.5, validation=True
        )


@pytest.mark.parametrize("perc_interactions_train", [0.5, 0.7])
def test_weak_generalization_split(data_m, perc_interactions_train):

    scenario = scenarios.WeakGeneralization(perc_interactions_train)
    ...
    perc_interactions_test = 1 - perc_interactions_train

    np.testing.assert_almost_equal(
        tr.values.nnz / data_m.values.nnz, perc_interactions_train, decimal=2
    )
    np.testing.assert_almost_equal(
...
@pytest.mark.parametrize(
    "perc_interactions_train, perc_interactions_validation", [(0.5, 0.25), (0.25, 0.25)]
)
def test_weak_generalization_split_w_validation(
    data_m, perc_interactions_train, perc_interactions_validation
):

    scenario = scenarios.WeakGeneralization(perc_interactions_train, perc_interactions_validation=perc_interactions_validation, validation=True)
    scenario.split(data_m)
    ...
    # Test approximately correct split
    perc_interactions_test = 1 - perc_interactions_train - perc_interactions_validation
    ...
    np.testing.assert_almost_equal(
        val_data_out.values.nnz / data_m.values.nnz, perc_interactions_validation, decimal=2
    )
