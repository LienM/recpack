...
def create_temporal_csr_matrix_from_pandas_df(df, item_ix, user_ix, timestamp_ix):
    column_ix = item_ix
    row_ix = user_ix
...
def create_data_M_from_pandas_df(df, item_ix, user_ix, timestamp_ix=None):
    """
    ...
    """
    values_sp_m = create_csr_matrix_from_pandas_df(df, item_ix, user_ix)
    timestamps_sp_m = None
    if timestamp_ix is not None:
        timestamps_sp_m = create_temporal_csr_matrix_from_pandas_df(df, item_ix, user_ix, timestamp_ix)
